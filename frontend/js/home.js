document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        searchResultItems.forEach(item => {
            const symbol = item.querySelector('.result-symbol').textContent.toLowerCase();
    ...
    searchResultItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            document.querySelector('.card-header h1').textContent = `Invest / ${name}`;
            ...
            document.querySelector('.price-value').textContent = `$${Number(price).toLocaleString()}`;
