...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateChartWithDummyData(timeframe) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        console.log("新的Y轴配置计算完成:");
        console.log("价格图表Y轴配置:", priceYAxis);
        console.log("蜡烛图表Y轴配置:", candlestickYAxis);
        console.log("交易量图表Y轴配置:", volumeYAxis);
       ...
       setTimeout(() => {
        window.candlestickChart.updateOptions({
          ...
          yaxis: candlestickYAxis  // 使用克隆后的配置
        }, true, true);  // 强制重绘并保持类型
        ...
        if (
          ...
          if (candleData && candleData.length > 0) {
            ...
            if (minPrice !== Infinity && maxPrice !== -Infinity) {
              ...
              config.min = Math.max(0, minPrice - padding); // 确保不会小于0，除非数据本身小于0
              config.max = maxPrice + padding;

              console.log(
                `Y轴范围(${chartType}): ${config.min.toFixed(
                  2
  ...
  function calculateChartHeight() {
    ...
    // 基础高度百分比 - 可以根据需要调整
    const lineChartHeightPercent = 0.18; // 视口的 20%
    const candleChartHeightPercent = 0.3; // 视口的 35%
  ...
  function setupCompanyTab() {
    ...
    const currentSymbol =
      document.getElementById("stockSymbol")?.textContent || "AAPL";
  ...
  const observer = new MutationObserver(function (mutations) {
    ...
    // This is a workaround for the initial mutation that occurs when the script loads
    ...
    for (const mutation of mutations) {
      if (mutation.attributeName === "data-theme") {
        const theme = document.body.getAttribute("data-theme");
        console.log(`MutationObserver 检测到主题变化为: ${theme}`);

        // 检查当前是否正在处理主题变化
        if (!window.isThemeChanging) {
          updateChartsTheme(theme);
        } else {
          console.log("已有主题更新正在进行，跳过重复更新");
        }
        break;
      }
    }
    ...
    this.resizeTimer = setTimeout(function () {
      ...
      if (window.priceChart) {
        window.priceChart.updateOptions({
          ...
          // 直接在这里更新Y轴配置，而不是调用 updateAllChartsYAxis
          yaxis: getYAxisConfig("price"),
        }, true, true);  // 强制重绘
      }
